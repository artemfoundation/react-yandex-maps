...
export class BaseObjectManager extends React.Component {
  constructor() {
    super();
  ...
  componentDidMount() {
    const { ymaps, name } = this.props;
    const instance = BaseObjectManager.mountObject(ymaps[name], this.props);
  ...
  componentDidUpdate(prevProps) {
    if (this.state.instance !== null) {
      BaseObjectManager.updateObject(
        this.state.instance,
        prevProps,
        this.props
      );
  ...
  componentWillUnmount() {
    BaseObjectManager.unmountObject(this.state.instance, this.props);
  }
...
export function createObjectManager(name) {
  const ObjectManager = props => <BaseObjectManager name={name} {...props} />;

  if (process.env.NODE_ENV !== 'production') {
    ObjectManager.displayName = name;
    ObjectManager.propTypes = BaseObjectManager.propTypes;
  }

  return withParentContext(withYMaps(ObjectManager, true, [name]));
}
